...
final class ExercisesTests: AbstractBaseTestsClass {
    ...
    func testRandomWordFilterByLevel() async throws {
        ...
        let query = ExercisesController.RandomWordQuery(language: try deutsch.requireID(), level: .B1)
    ...
    func testRandomWordFilterByLevelAndType() async throws {
        ...
        let query = ExercisesController.RandomWordQuery(language: try deutsch.requireID(), level: .A1, type: .verb)
    ...
    func testRandomWordFilterWithEmptySelects() async throws {
        ...
        XCTAssertContains(res.body.string, "<input type=\"hidden\" name=\"type\" value=\"\">")
        XCTAssertContains(res.body.string, "<input type=\"hidden\" name=\"level\" value=\"\">")
    }
