...
private func genericWordSampleCreation(languageId: UUID, word: String, rawWord: UUID?=nil, level: WordLevel?=nil, type: WordType, declinations: [String:[String]], on db: Database) async throws {
    
    let word = Word(word: word, type: type, level: level, rawID: rawWord, languageID: languageId)
    try await word.save(on: db)
    for (declinatedWord, decNames) in declinations{
        let declinationTypeCases = try await DeclinationTypeCase.query(on: db).filter(\.$name ~~ decNames).all()
...
func createSampleWords(app: Application, language: UUID) async throws {
    ...
    try await genericWordSampleCreation(languageId: language, word: "Haus", rawWord: rawHause.id, level: .A1, type: .noun, declinations: [
        "Haus": ["das"]], on: app.db)
    
    try await genericWordSampleCreation(languageId: language, word: "sein", level: .A1, type: .verb, declinations: [
        "bin": ["ich", "present"],
        "bist": ["du", "present"],
